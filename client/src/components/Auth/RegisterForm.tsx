...
const RegisterForm: React.FC = () => {
  ...
  const checkEmail = async () => {
    ...
    setEmailError('');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}/auth/validate-email?email=${encodeURIComponent(formData.email)}`);
      ...
      // If validation service fails, do not block user, but show a hint
      setEmailError('Could not verify email domain. Please double-check.');
      return true;
    } finally {
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {emailChecking && (
                <p className="text-xs text-gray-500 mt-1">Validating email…</p>
              )}
              {emailError && (
